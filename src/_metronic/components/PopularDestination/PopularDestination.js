...
const PopularDestination = ({ getNewCount, title }) => {
  ...
  useEffect(() => {
    title === "Dashboard | OUR LEISURE HOME"
      ...
      : (document.title = "Learn Page-Share the Joy | OUR LEISURE HOME");
  }, []);
  ...
  const getAllAboutUs = async () => {
    ...
    if (!search) {
      await ApiGet(
        `aboutus/getAllAboutus?page=${page}&limit=${countPerPage}&type=populardestination`
      )
        .then((res) => {
      ...
      await ApiGet(
        `aboutus/getAllAboutus?search=${search}&page=${page}&limit=${countPerPage}&type=populardestination`
      )
        .then((res) => {
          setIsLoaderVisible(false);
          setFilteredAboutUs(res?.data?.payload?.aboutus);
        ...
        <div className="p-2 mb-2">
          <div className="row mb-4 pr-3">
            <div className="col-md-3 col-sm-12 d-flex justify-content-between">
              <h2 className="pl-3 pt-2">Learn Page-Share the Joy</h2>
            </div>
            <div className="col-md-7 col-sm-12 mobile-view-align">
              <div>
                <input
                  ...
                  name="title"
                  placeholder="Search Learn Page-Share the Joy"
