...
const StandardAmenities = ({ getNewCount, title }) => {
  ...
  useEffect(() => {
    title === "Dashboard | OUR LEISURE HOME"
      ...
      : (document.title = "Standard Amenities | OUR LEISURE HOME");
  }, []);
  ...
  const getAllAmeninties = async () => {
    ...
    if (!search) {
      await ApiGet(
        `amenities/getAmenities?type=STANDARD&page=${page}&limit=${countPerPage}`
      )
        .then((res) => {
      ...
      await ApiGet(
        `amenities/getAmenities?type=STANDARD&search=${search}&page=${page}&limit=${countPerPage}`
      )
        .then((res) => {
          setIsLoaderVisible(false);
          setAmenintiesData(res?.data?.payload?.Ameninties);
  ...
  const validateFormForAddAdmin = () => {
    ...
    if (inputValueForAdd && !inputValueForAdd.name) {
      formIsValid = false;
      errorsForAdd["name"] = "*Please enter name of standard amenity";
    }
    if (!iconData) {
      formIsValid = false;
      errorsForAdd["icon"] = "*Please upload standard amenity icon";
    }
  ...
  const handelAddAmenintiesDetails = (e) => {
    e.preventDefault();
    if (validateFormForAddAdmin()) {
      ...
      ApiPost(`amenities/addAmenities`, Data)
        .then((res) => {
          if (res?.status == 200) {
            setIsAddAmeninties(false);
            toast.success("Standard amenity added successfully");
            setInputValueForAdd({});
            getAllAmeninties();
            {
  ...
  const validateForm = () => {
    ...
    if (inputValue && !inputValue.name) {
      formIsValid = false;
      errors["name"] = "*Please Enter Name of Amenintie!";
    }
    if (!iconData) {
      formIsValid = false;
      errors["icon"] = "*Please upload standard amenity icon";
    }
    setErrors(errors);
    return formIsValid;
  };
  ...
  const handleDeleteAmenintie = () => {
    ApiDelete(`amenities/deleteAmenities/${idForDeleteAmeninties}`)
      .then((res) => {
        if (res?.status == 200) {
          setShow(false);
          toast.success("Standard amenity deleted successfully");
          getAllAmeninties();
          {
    ...
    if (validateForm()) {
      ...
      ApiPut(`amenities/updateAmenities/${idForUpdateAmenintiesData}`, data)
        .then((res) => {
          if (res?.status == 200) {
            setIsUpdateAmeninties(false);
            toast.success("Standard amenity updated successfully");
            setInputValue({});
            getAllAmeninties();
          } else {
            toast.error(res?.data?.message);
        ...
        <div className="p-2 mb-2">
          <div className="row mb-4 pr-3">
            <div className="col-md-3 col-sm-12 d-flex justify-content-between">
              <h2 className="pl-3 pt-2">Standard Amenities</h2>
            </div>
            <div className="col-md-7 col-sm-12 mobile-view-align">
              <div>
                <input
                  ...
                  name="title"
                  placeholder="Search Standard Amenities"
          ...
          <Modal show={show} onHide={handleClose}>
            ...
            <Modal.Body>
              Are you sure you want to delete this standard amenity ?
            </Modal.Body>
            <Modal.Footer>
...
export default StandardAmenities;
