...
const HomePage = ({ getNewCount, title }) => {
  ...
  useEffect(() => {
    title === "Dashboard | OUR LEISURE HOME"
      ...
      : (document.title =
        "Home Developer-DON'T JUST WISH, OWN IT. | OUR LEISURE HOME");
  }, []);
  ...
  const getAllAboutUs = async () => {
    setIsLoaderVisible(true);
      await ApiGet(
        `aboutus/getAllAboutus?page=${page}&limit=${countPerPage}&type=homepage`
      )
        .then((res) => {
            ...
            {/* <div className="col-md-7 col-sm-12 mobile-view-align">
              <div>
                <input
                  ...
                  name="title"
                  placeholder="Search Home Developer-DON'T JUST WISH, OWN IT."
