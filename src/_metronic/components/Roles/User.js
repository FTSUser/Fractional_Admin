...
const User = ({ title }) => {
  ...
  useEffect(() => {
    title === "Dashboard | OUR LEISURE HOME"
      ...
      : (document.title = "User | OUR LEISURE HOME");
  }, []);
  ...
  const getAllAdmins = async () => {
    ...
    if (!search) {
      await ApiGet(`admin/get-users?isActive=true&page=${page}&limit=${countPerPage}`)
        .then((res) => {
      ...
      await ApiGet(
        `admin/get-users?isActive=true&search=${search}&page=${page}&limit=${countPerPage}`
      )
        .then((res) => {
          setIsLoaderVisible(false);
          setFilteredAdmin(res?.data?.payload?.user);
  ...
  const columns = [
    ...
    //             <>
    //                 {row?.locked === true ?
    //                     <div onClick={() => { handleShowImage(row); setIdForEditStatus(row?._id) }} >
    //                         <p>Yes</p>
    //                     </div>
    //                     :
    //                     <p>No</p>
        ...
        <div className="p-2 mb-2">
          <div className="row mb-4 pr-3">
            <div className="col-md-3 col-sm-12 d-flex justify-content-between">
              <h2 className="pl-3 pt-2">User Listing</h2>
            </div>
          ...
          <Modal show={showModal} onHide={handleModalClose}>
            ...
            <Modal.Body>
              <div className="d-flex justify-content-between">
                Are you sure you want to{" "}
                {statusForActive === true ? "active" : "inactive"} this user ?
              </div>
            ...
            <Modal.Footer>
              ...
              >
                {statusForActive === true ? "Active" : "Inactive"}
              </Button>
            </Modal.Footer>
          </Modal>
